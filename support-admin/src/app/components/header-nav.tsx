...
export default function HeaderNav({
  ...
  const showPasswordChange = role !== "user";
  ...
  return (
    <>
      ...
      <button
        ...
        aria-label="Меню"
      >
        <span className={`block h-0.5 w-5 bg-gray-700 dark:bg-gray-300 rounded transition-transform ${open ? "translate-y-2 rotate-45" : ""}`} />
        <span className={`block h-0.5 w-5 bg-gray-700 dark:bg-gray-300 rounded transition-opacity ${open ? "opacity-0" : ""}`} />
        <span className={`block h-0.5 w-5 bg-gray-700 dark:bg-gray-300 rounded transition-transform ${open ? "-translate-y-2 -rotate-45" : ""}`} />
      </button>
